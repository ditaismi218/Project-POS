...
class AbsensiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Ambil tanggal dari request kalau ada
    ...
    public function import(Request $request)
    {
        ...
        $data = Excel::toArray(new AbsensiImport, $file)[0];

        foreach ($data as $row) {
            $user = User::where('name', $row['nama'])->first();

            if ($user) {
                // Cek apakah sudah ada data absensi untuk user & tanggal
                $sudahAda = Absensi::where('user_id', $user->id)
                    ->whereDate('tanggal_masuk', $row['tanggal_masuk'])
                    ->exists();
                ...
                $waktuMasuk = in_array($row['status_masuk'], ['sakit', 'cuti']) ? '00:00:00' : $row['waktu_masuk'];
                $waktuSelesai = in_array($row['status_masuk'], ['sakit', 'cuti']) ? '00:00:00' : ($row['waktu_selesai_kerja'] ?? null);
