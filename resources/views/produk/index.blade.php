                    ...
                    <tbody>
                        @foreach ($products as $index => $item)
                            <tr>
                                ...
                                <td>
                                    {!! \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->kode_barang, 'C128') !!}
                                ...
                                <td>{{ $item->satuan }}</td>
                                <td>
                                    <button class="btn btn-warning btn-sm d-flex align-items-center mb-2 edit-button"
                                        data-id="{{ $item->id }}" data-kode="{{ $item->kode_barang }}"
                                        data-nama="{{ $item->nama_barang }} " data-kategori="{{ $item->kategori_id }}"
                                        data-gambar="{{ $item->gambar }}" data-satuan="{{ $item->satuan }}"
                                        data-bs-toggle="modal" data-bs-target="#productModal">
                                        <i class="bx bx-edit"></i>
                                    </button>

                                    @if ($item->penerimaanBarang->count() == 0)
                                        <button class="btn btn-danger btn-sm d-flex align-items-center gap-2 delete-button"
                                            data-id="{{ $item->id }}" data-nama="{{ $item->nama_barang }}">
                                            <i class="bx bx-trash"></i> <!-- Ikon untuk Hapus -->
                                        </button>
                                    @else
                                        <button class="btn btn-secondary btn-sm d-flex align-items-center gap-2" disabled
                                            title="Tidak bisa dihapus karena sudah digunakan">
                                            <i class="bx bx-lock-alt"></i> <!-- Ikon untuk Dipakai -->
                                        </button>
                                    @endif
                                </td>
                            </tr>
                ...
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // Kirim permintaan DELETE melalui form
