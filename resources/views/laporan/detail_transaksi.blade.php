...
<div class="card">
    <div class="card-body">
        <div id="printArea" class="p-3">
                    ...
                    @foreach ($transaksi->detailPenjualan as $detail)
                        @php
                            ...
                            if (!isset($groupedProducts[$namaProduk])) {
                                $groupedProducts[$namaProduk] = ['qty' => 0, 'subtotal' => 0];
                            }
            ...
            <table class="table table-borderless">
                <tr>
                    ...
                    <td class="text-end fw-bold text-primary">Rp {{ number_format($transaksi->total_bayar, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td><strong>Jumlah Bayar</strong></td>
                    <td class="text-end">Rp {{ number_format($transaksi->pembayaran->jumlah_bayar ?? 0, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td><strong>Kembalian</strong></td>
                    <td class="text-end text-success">Rp {{ number_format($transaksi->pembayaran->kembalian ?? 0, 0, ',', '.') }}</td>
                </tr>
                <tr>
        ...
        <div class="text-center mt-4">
            ...
            <a href="{{ route('laporan.transaksi') }}" class="btn btn-secondary btn-lg"><i class="fas fa-arrow-left"></i> Kembali</a>
        </div>
    </div>
</div>
